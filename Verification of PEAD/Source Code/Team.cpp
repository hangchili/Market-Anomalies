...
{
	AAR.resize(240);
	Team SampleList;//need a team default constructor have a empty map
	srand(time(NULL));
	for (int t = 0; t < 5; t++)
	{
		...
		SampleList.GenAAR();//AAR is changing after every sample
		AAR = (AAR * t) / (t + 1) + SampleList.AAR / (t + 1);
		SampleList.StockList.clear();
	}
	GenCAAR();
}
...
void GenReturnMatrix(Team &Beat, Team &Meet, Team&Miss, ReturnMatrix &TeamReturn)
{
	...
	Team *ptr = &ThreeGroup[0];
	for (int i = 0; i < 3; i++)
	{
		ptr->RandSelect();
		TeamReturn[i][0].assign(ptr->AAR.begin(), ptr->AAR.end());
		TeamReturn[i][1].assign(ptr->CAAR.begin(), ptr->CAAR.end());
		ptr++;
	}
}
